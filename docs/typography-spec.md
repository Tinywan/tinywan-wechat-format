# 文章排版系统（输出侧）设计规范

> 适用对象：`md-wechat-formatter` 将 Markdown 渲染为微信公众号正文 HTML 的输出样式系统。
> 单一事实来源：`src/core/themes.js`（样式 token 工厂）+ `src/core/render.js`（markdown-it 规则）。
> 默认示例取值均基于默认主题「蓝色科技」（blue-tech）。

---

## 1. 设计原则

1. **纯内联样式**：输出不含任何 `class`、`<style>`、外链 CSS —— 微信编辑器会剥离它们。
2. **字体自包含**：每个文字元素自带完整字体栈，不依赖继承（微信会打断继承链）。
3. **渐变必降级**：所有 `linear-gradient` 前先写同位置纯色，旧内核回退到纯色。
4. **结构扁平化**：不使用 `ul/ol/li`（微信粘贴易丢失），列表转为「段落 + 标记」。
5. **主题可插拔**：渲染规则与颜色解耦，调色板 → 工厂 → 全量 token，一键换肤。

---

## 2. 字体

| Token | 取值 | 用途 |
|---|---|---|
| `FONT` | `'HarmonyOS Sans','PingFang SC','MiSans','Source Han Sans SC','Microsoft YaHei',-apple-system,sans-serif` | 所有正文、标题、表格、图注 |
| `MONO` | `Consolas,Menlo,monospace` | 行内代码、代码块 |

---

## 3. 容器

整篇文章包裹在 `<section>` 中：

```css
max-width: 677px;   /* 微信正文经典阅读宽度 */
margin: 0 auto;
padding: 0 8px;
```

---

## 4. 标题

| 级别 | 字号 | 行高 | 颜色 | 对齐 | 其他 |
|---|---|---|---|---|---|
| h1 | 22px | 2 | `deep` | 居中 | margin `28px 0 24px` |
| h2 | 20px | 1.8 | `deep` | 居中 | margin `40px 0 22px`，支持数字色块 |
| h3 | 17px | 1.6 | `primary` | 左 | 左侧 4px `primary` 竖条，`padding-left:10px`，margin `0 0 16px` |
| h4 | 16px | 1.8 | `primary` | 左 | margin `0 0 14px` |

所有标题 `font-weight:bold`。

### h2 数字色块（chip）

h2 以数字开头（匹配 `^(\d{1,2})[\s、.．·]+`，如 `1、`、`2.`、`3 `）时，数字被抽出为渐变色块：

```css
display:inline-block; padding:0 10px; border-radius:6px;
background:#2273b8;                                  /* 纯色降级 */
background:linear-gradient(135deg, #2273b8, #3a8ee6);
color:#ffffff; font-size:16px; line-height:1.7;
margin-right:10px; vertical-align:middle;
```

实现要点：core rule 在解析期产出中性 token（`h2_chip`/`h2_chip_close`），渲染期才套用 env 主题 —— 解析与配色解耦。

---

## 5. 正文段落

```css
font-size:16px; line-height:2; letter-spacing:1px;
color:#333; text-align:justify;
margin:0 0 20px 0;
```

- `text-align:justify` 两端对齐，中文排版更整齐；
- `letter-spacing:1px` 补偿微信端字距压缩。

---

## 6. 行内元素

| 元素 | 输出 | 说明 |
|---|---|---|
| `**加粗**` | `<strong>` | 引用块内加粗着 `primary` 色 |
| `*斜体*` | `<em>` | 原样输出 |
| `[链接](url)` | 主题色加粗 `<span>` | 公众号正文外链无效，丢弃 href：`color:${primary};font-weight:bold` |
| 软/硬换行 | `<br>` | — |

### 行内代码

```css
background:#e8edf2; color:#4a7fa5;   /* blue-tech 取值 */
border-radius:3px; padding:1px 6px;
font-size:13px; font-family:Consolas,Menlo,monospace;
```

---

## 7. 列表

微信不保留 `ul/ol`，统一输出为 `<p>` + 标记。列表项公共样式：

```css
font-size:16px; line-height:2; letter-spacing:1px; color:#333;
text-align:justify;
padding-left:16px; text-indent:-16px;   /* 悬挂缩进：折行后文字对齐首行文字 */
margin:0 0 18px 0;
```

### 无序列表：▸ 字符标记

```css
color:#2273b8; font-weight:bold; margin-right:6px;
/* 内容：▸ */
```

### 有序列表：主题色数字

```html
<span style="color:#2273b8;font-weight:bold;">1.</span>&nbsp;
```

- 计数器由渲染器维护（支持嵌套，各层独立计数）；
- 嵌套层级用 4 个 `\u00a0` × 层深做缩进。

---

## 8. 引用块

输出为 `<section>` 卡片（非 `<blockquote>`）：

```css
/* 外框 */
background:#eaf2fa;                     /* quoteBg */
border-left:4px solid #2273b8;
border-radius:4px; padding:14px 16px; margin:0 0 20px 0;

/* 段落 */
font-size:15px; line-height:1.9; letter-spacing:1px;
color:#333; margin:0;                    /* 末段 */
/* 非末段 margin 改为 0 0 8px，段间留缝 */
```

特殊规则：

| 规则 | 说明 |
|---|---|
| 签名行 | 以 `——` 开头的段落：13px、`gray` 色（`#8a94a0`）、line-height 1.8 |
| 加粗着色 | 引用内 `**加粗**` 着 `primary` 色 |
| 字距 | 引用文字同样 `letter-spacing:1px`，与正文字感一致 |

---

## 9. 图片与图注

```css
/* 图容器 */
text-align:center; margin:0 0 6px 0; font-size:0; line-height:0;

/* 图片 */
max-width:100%; display:block; margin:0 auto;

/* 图注 */
font-size:12px; line-height:1.8; color:#8a94a0;
text-align:center; margin:0 0 20px 0;
```

规则：

1. **base64 内嵌**：图片按文件名匹配用户上传的本地文件，命中则替换为 base64（微信不加载外链图）；
2. **图注自动生成**：`图 N · alt`（alt 已含 `图 N ·` 前缀则不重复编号），N 为全文递增序号；alt 为空时取文件名；图注前带 `primary` 色 `▍` 竖条；
3. **未匹配警示**：未命中的图片加红色虚线外框（`outline:2px dashed #e06c75`）并计入顶部警告条。

---

## 10. 代码块（窗口风格）

```css
/* 外框 */
background:#2a2f3a;                     /* codeBg */
border-radius:6px; padding:16px 14px; margin:0 0 20px 0;
```

### 头部（fence 带语言/标题时显示）

- 三个 macOS 信号点：`#ff5f57` / `#febc2e` / `#28c840`，10px 圆点；
- 标签：`LANG · 标题`（或仅语言名大写），11px `MONO`，`codeLabel` 色。

示例 fence：` ```php app.php ` → 标签 `PHP · app.php`。

### 代码行

```css
font-size:13px; line-height:1.9; color:#aeb6c2;   /* codeText */
font-family:Consolas,Menlo,monospace;
word-break:break-all;                              /* 长行折行，手机不溢出 */
margin:0;
```

- 空格硬化为 `\u00a0` 保对齐（高亮管线处理）；
- **首行注释标题化**：若首行整行是注释，套用 `titleLine`：加粗、着 `codeTitleColor`（如 `#c678dd`），作为代码块的"标题"。

### 语法高亮色板

| 语义 | One Dark（深色底） | GitHub Light（浅色底） |
|---|---|---|
| comment / quote | `#7f848e` | `#6a737d` |
| string / number / attr | `#d19a66` | `#032f62` |
| keyword / meta | `#c678dd` | `#d73a49` |
| title / function | `#61afef` | `#6f42c1` |
| class / type / built_in | `#e5c07b` | `#e36209` |
| variable / tag | `#e06c75` | `#005cc5` |
| symbol / bullet / addition | `#98c379` | `#22863a` |

---

## 11. 表格

```css
/* table */
border-collapse:collapse; width:100%;
margin:0 0 20px 0;

/* th */
background:#2273b8; color:#ffffff;
font-size:16px; font-weight:bold; line-height:1.7; letter-spacing:1px;
padding:10px 12px; border:1px solid #2273b8; text-align:left;

/* td */
font-size:16px; line-height:1.7; color:#333;
padding:10px 12px; border:1px solid #d5e2ef;   /* tableBorder */
text-align:left; vertical-align:top;
```

---

## 12. 分隔线

```css
border:none; height:3px; border-radius:2px;
background:#2273b8;                                      /* 纯色降级 */
background:linear-gradient(90deg, #2273b8, #3a8ee6);
width:60px; margin:32px auto;
```

短粗居中渐变条，替代默认灰线。

---

## 13. 主题系统

### 工厂模式

`buildTheme(palette)` 接收约 15 个调色板值，展开为全部输出 token：

```js
palette: {
  primary, deep, light,       // 主色 / 深色（h1、h2 文字）/ 亮色（渐变端点）
  accent,                     // 备用强调色
  quoteBg,                    // 引用底色
  codeBg, codeText, codeLabel, codeTitleColor,
  inlineCodeBg, inlineCodeText,
  tableBorder,
  syntax: oneDark | githubLight,
  // 可选覆盖：text, gray, quoteText, quoteGray（深色主题需要）
}
```

### 五套主题

| id | 名称 | primary | deep → light | 引用底 | 代码底 | 行内代码 | 表边框 | 语法色板 |
|---|---|---|---|---|---|---|---|---|
| `blue-tech` | 蓝色科技 | `#2273b8` | `#1f5fa6 → #3a8ee6` | `#eaf2fa` | `#2a2f3a` | `#e8edf2/#4a7fa5` | `#d5e2ef` | One Dark |
| `orange-heart` | 橙心 | `#ff3502` | `#e6461f → #ff8052` | `#fff2ec` | `#2d2a26` | `#ffece3/#d4380d` | `#ffd9cc` | One Dark |
| `violet` | 姹紫 | `#5d3587` | `#4a2a6b → #8e5fc0` | `#f4eef9` | `#2b2436` | `#efe6f7/#6b3fa0` | `#e2d5f0` | One Dark |
| `green` | 绿意 | `#0e8c64` | `#0a6b4c → #3fb88f` | `#eaf7f1` | `#f6f8fa`（浅） | `#e6f4ef/#0e8c64` | `#cfe8df` | GitHub Light |
| `night` | 凝夜 | `#b39ddb` | `#5e35b1 → #7e57c2` | `#232733` | `#14161c` | `#2b3040/#b39ddb` | `#3a4152` | One Dark |

「凝夜」为暗色主题，额外覆盖 `quoteText:#cfd4e3`、`quoteGray:#8b93a7`，避免浅字落在深底上不可读。

### 换肤链路

```
用户选择 → themeId（localStorage 持久化）
→ getTheme(id) → render(src, { theme }) → env.theme → 全部规则 th(env)
```

---

## 14. 微信兼容约束清单

| 约束 | 实现 |
|---|---|
| 仅内联 style | 渲染器只产 `style="…"`，无 class、无 `<style>` |
| 不用 ul/ol/li | 列表 → `<p>` + 标记 span（见 §7） |
| 渐变降级 | 纯色在前、渐变在后（chip、圆点、分隔线） |
| 空格保形 | 代码空格 → `\u00a0` |
| 长行不溢出 | 代码行 `word-break:break-all` |
| 外链无效 | 链接 → 主题色加粗文本，丢弃 href |
| 外链图无效 | 图片 → base64 内嵌（文件名匹配） |
| 字体继承被打断 | 每个文字元素显式声明完整字体栈 |

---

## 15. 源码映射

| 文件 | 职责 |
|---|---|
| `src/core/themes.js` | 字体常量、语法色板、`buildTheme` 工厂、5 主题注册表、`getTheme` |
| `src/core/render.js` | markdown-it 规则：h2 色块、引用卡片、列表标记、代码窗口、表格、图注、容器包裹 |
| `src/core/highlight.js` | hljs 高亮 → 逐行 token span（空格硬化在此） |
| `src/composables/useFormatter.js` | themeId 状态（localStorage）、图片匹配、渲染调度 |
| `test-render.mjs` | 回归测试（~34 项）：结构合规、多主题取色、表格、折行、图注格式 |
